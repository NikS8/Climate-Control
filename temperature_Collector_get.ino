...
DeviceAddress sensorDsCollectorIn = { 0x28, 0xFF, 0x74, 0xF0, 0xB2, 0x16, 0x03, 0x1C };   // адрес датчика DS18B20 на входе в коллектор
DeviceAddress sensorDsCollectorOut = { 0x28, 0xFF, 0x57, 0x1E, 0xB3, 0x16, 0x04, 0xDE };  // адрес датчика DS18B20 на выходе из коллектора
DeviceAddress sensorDsKitchenOut1 = { 0x28, 0xFF, 0x63, 0x89, 0x01, 0x17, 0x05, 0x4B };    // адрес датчика DS18B20 на трубе №1 из летней кухни
DeviceAddress sensorDsKitchenOut2 = { 0x28, 0xFF, 0x14, 0x17, 0x01, 0x17, 0x03, 0x5E };    // адрес датчика DS18B20 на трубе №2 из летней кухни
...
int sensorDsTempKitchenOut1;    // температура датчика DS18B20 на трубе №1 из летней кухни
int sensorDsTempKitchenOut2;    // температура датчика DS18B20 на трубе №2 из летней кухни
...
struct SEND_DATA_STRUCTURE {                  // структура, которую будем передавать
  int ID;
  int sensorDsKitchenOut1;    // адрес датчика DS18B20 на трубе №1 из летней кухни
  int sensorDsKitchenOut2;    // адрес датчика DS18B20 на трубе №2 из летней кухни
...
char action;    //  "get"/"set" - запрос выдачи данных / команда на исполнение
int levelPin;   //  установка уровня на Pin (LOW/HIGH)
...
void loop() {
  ...
  delay(30);
  sensorDsTempKitchenOut1 = sensorsDS.getTempC(sensorDsKitchenOut1);    // температура датчика DS18B20 на трубе №1 из летней кухни
  sensorDsTempKitchenOut2 = sensorsDS.getTempC(sensorDsKitchenOut2);    // температура датчика DS18B20 на трубе №2 из летней кухни
  sensorDsTempCollectorIn = sensorsDS.getTempC(sensorDsCollectorIn);   // температура датчика DS18B20 на входе в коллектор
  sensorDsTempCollectorOut = sensorsDS.getTempC(sensorDsCollectorOut);  // температура датчика DS18B20 на выходе из коллектора
  ...
  if (ETin.receiveData()) {                         // если пришли данные
    ...
    if (id == ID) { // и если пакет пришел нам
      ...
      else
      {
        ...
        txdata.ID = ID;
        txdata.sensorDsKitchenOut1 = sensorDsTempKitchenOut1;    // температура датчика DS18B20 на трубе №1 из летней кухни
        txdata.sensorDsKitchenOut2 = sensorDsTempKitchenOut2;    // температура датчика DS18B20 на трубе №2 из летней кухни
